...
import { storageJson } from './samples.storage.js';
...
var SAMPLES: TraceSample[] = [
...

  {
    ...
    /* 資料本體的唯一來源是 samples/storage.json（原封不動取自參考 repo），由
       scripts/gen-samples.mjs 產生成 samples.storage.ts。以前這裡手抄了一份同樣的資料
       （425 行），註解寫著「改一邊記得改另一邊」——那種註解就是漂移的前兆，已經拿掉。 */
    json: storageJson
  }
];
