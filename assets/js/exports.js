...
(function (global) {
  ...
  function mermaidSankey(model, mode) {
    var attr = mode === 'contribution';
  ...
  function mermaidFlow(model, mode) {
    var attr = mode === 'contribution';
    var L = ['flowchart LR'];
    model.nodes.forEach(function (n) {
      if (n.kind === 'node') {
        ...
        var lt = '追查終止<br/>' + n.label + (n.namespace ? '<br/>ns/' + n.namespace : '') +
          '<br/>' + F(attr ? n.attr : n.bps) + '<br/>未再往下追';
